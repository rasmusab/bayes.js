...
test_that("real_adaptive_metropolis_sampler works", {
  ...
  j$eval("var sampler = real_adaptive_metropolis_sampler('x', -Infinity, Infinity, state, posterior)")
  norm_samples = replicate(10000, j$get("sampler.next()"))
...
test_that("int_adaptive_metropolis_sampler works", {
  ...
  j$eval("var sampler = int_adaptive_metropolis_sampler('x', 0, Infinity, state, posterior)")
  poisson_samples = replicate(10000, j$get("sampler.next()"))
...
test_that("real_multivariate_adaptive_metropolis_sampler works", {
  ...
  j$eval("var options = {min_adaptation: 0.2, prop_log_scale: [[10,0],[-10, 5]]};")
  j$eval("var sampler = real_multivariate_adaptive_metropolis_sampler('x', -Infinity, Infinity, [2, 2] ,state, posterior, options)")
  norm_samples = replicate(100, j$get("sampler.next()"))
  j$eval("sampler.stop_adaptation()")
  norm_samples = replicate(1000, j$get("sampler.next()"))
  j$eval("sampler.start_adaptation()")
  norm_samples = replicate(10000, j$get("sampler.next()"))
  norm_samples = replicate(10000, j$get("sampler.next()"))
  norm_samples = norm_samples[, , sample(1:10000, 1000)]
